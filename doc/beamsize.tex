...
length is the Gaussian standard deviation $\major$ radians, and
$\beta$ is the coordinate along the minor axis, whose length is the
Gaussian standard deviation, $\minor$ radians.
...
north by an angle $\bpa$ (radians). Within Difmap, however, images of
the sky are stored with coordinates of $(x,y)$, where $x$ goes from
west to east. In this coordinate system the beam is rotated clockwise
by the angle $\bpa$.

Given a sky coordinate, $(x,y)$, expressed in radians, the
corresponding position along the major and minor axes of the
elliptical Gaussian are given by rotating the $x$ and $y$ clockwise by
the angle $\bpa$, as follows.
...
If the Fourier transform of $s(x,y)$ is denoted $S(u,v)$, where $x$
and $y$ are in radians, and $u$ and $v$ are numbers of wavelengths at
the observing frequency, then the image plane derivatives needed in
the previous section can be obtained as follows.
