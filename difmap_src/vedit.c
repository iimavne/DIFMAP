...
enum {
  ...
  /*  A.Collioud - 2014/03/20 */
  KEY_SMARTFLG='Y' /* Smart automatic flagging within a scan (only one baseline at a time) */
};
...
/*.......................................................................
 * Smart automatic flagging within the scan and baseline selected by the cursor.
 *
 * Input:
 *  vp      Vedpar *  The visibility plot attributes to supply to visplt().
 *  vs      Vissub *  The sub-plot descriptor returned by v_cursor().
 ...
 *  return     int    0 - OK. Anything else means fatal error.
 *
 * A.Collioud - collioud@obs.u-bordeaux1.fr - 2014 March 20
 * !! Copy of v_zap !!
 */

static int v_smart(Vedpar *vp, Vissub *vs, Scan *scan, int doflag)
{
  int t;          /* Time index loop variable */
  int save_mode;  /* Used to record station/baseline editing mode */
  int save_if_mode;  /* Used to record all/single IF editing mode */
  double tmin;    /* Start of the selected time range */
  double tmax;    /* End of the selected time range */
  int ierr=0;     /* Error status flag */
  ...
  double stdev_mean=0;
  double median=0;
  double stdev_median=0;
  
  double meanphserr=0;
  double stdev_meanphserr=0;
  double medianphserr=0;
  double stdev_medianphserr=0;
  ...
  int i=0;
  double visamp[vp->tb]; /* vis amplitude vector. vp->tb + 1 == nb of visibilities in the scan */
  double visamperr[vp->tb]; /* vis amplitude error vector */
  double visphs[vp->tb]; /* vis phase vector */
  double visphserr[vp->tb]; /* vis phase error vector */
  
  double absdevtomedian[vp->tb];
 ...
 */
  ...
  vp->stat_ed = 0;
/*
 * Turn off all If editing.
 */
  save_if_mode = vp->if_ed;
  vp->if_ed = 1;
/*
 * Only flag points in the time range selected under certain conditions (amp, phase, ...)
 * to determine !
 */
  
 /*
  lprintf(stderr,"ta= %i, tb = %i\n",vp->ta,vp->tb);
  */
  /* The indexes of the first and last plotted  integrations.
  <=> vp->tb + 1 = nb of visibilities (including null values)
  */
  
  for(t=vp->ta; !ierr && t<=vp->tb; t++) {
    
    /*lprintf(stderr,"t = %i\n",t);*/
    /* t= int from 0 to nb of vis in a scan*/
  
    TimeSample *sample = vp->times + t;
    float tval = sample->t;
    /*lprintf(stderr,"tval = %f\n",tval); */
    /* Real time value in sec
          ex: d0: 79976.0 s -> 22h12m56s
	      d1: 146671.0 s -> 40h44m31s <=> 16h44m31s [24]
    */
    
    if(tval >= tmin && tval <= tmax) {  /* Only flag in given time range */
      Visibility *vis = sample->integ->vis + vs->base;
            
      /*lprintf(stderr,"vis = %f ** %f (wt=%f)\n",vis->amp,vis->phs,vis->wt);*/
      /* Amp ; phase (radians) ; weight (1/variance)*/
      

/*
 * Ignore deleted points.
 */
      /*lprintf(stderr,"flag = %d\n",vis->bad);*/
      /* if 0, not flagged ; if 2, flagged.*/
      if(vis->bad & FLAG_DEL){
	continue;
      };
      if(vis->bad){
	continue;
      }; /* allow mutiple run of smart editing */
      
      /* Load amplitude>phase vector*/
      visamp[i] = vis->amp;
      visamperr[i] = 1.0f/sqrt(fabs(vis->wt));
      
      visphs[i] = vis->phs;
      visphs[i] -= twopi*floor(visphs[i]/twopi+0.5);  /* Wrap into -pi to pi range */
      if(visamp[i] > 1.0e-20f)
        visphserr[i] = visamperr[i]/visamp[i];
      else
        visphserr[i] = 0.0f;
  ...
  int v=0;
  for(v;v<i;v++){
    lprintf(stderr,"vis(%d): amp=%f (err %f); phs=%f (err %f)\n",v,visamp[v],visamperr[v],visphs[v],visphserr[v]);
  };
  */
   
  /* Calculate statistics */
  lprintf(stderr,"nbvis=%d\n",i);
  
  lprintf(stderr,"* Amplitude\n");
  mean = gsl_stats_mean(visamp,1,i);
  stdev_mean = gsl_stats_sd_with_fixed_mean(visamp,1,i,mean);
  
  /* 1- STANDARD DEVIATION METHOD */
  lprintf(stderr,"1- STANDARD DEVIATION METHOD\n");
  lprintf(stderr,"mean=%lf, stdev=%lf\n",mean,stdev_mean);
  lprintf(stderr,"+/- 2 stdev = %lf ; %lf\n",mean-2.0f*stdev_mean,mean+2.0f*stdev_mean);
  lprintf(stderr,"+/- 3 stdev = %lf ; %lf\n",mean-3.0f*stdev_mean,mean+3.0f*stdev_mean);
  lprintf(stderr,"\n");  
  
  /* 1b- MADe METHOD */
  /* MAD=median{|xi-median|} ; Median Absolute Deviation */
  /* MADe=1.4826*MAD */
  lprintf(stderr,"1b- MADe METHOD\n");
  gsl_sort(visamp,1,i);
  median = gsl_stats_median_from_sorted_data (visamp, 1, i);
  
  int k=0;
  for(k;k<i;k++){
    absdevtomedian[k]=fabs(visamp[k]-median);
  };
  gsl_sort(absdevtomedian,1,i);
  MAD = gsl_stats_median_from_sorted_data (absdevtomedian, 1, i);
  MADe = 1.4826*MAD;  
  /* this constant is for constitency with normal distribution */
  
  lprintf(stderr,"median=%lf\n",median);
  lprintf(stderr,"+/- 2 MADe = %lf ; %lf\n",median-2.0f*MADe,median+2.0f*MADe);
  lprintf(stderr,"+/- 3 MADe = %lf ; %lf\n",median-3.0f*MADe,median+3.0f*MADe);
  lprintf(stderr,"\n");
    
  /* 2- Z-SCORE */
  /* Z = (xi-mean)/stdev
   * if |Z| > 3 => outlier ; equivalent to 3sig
   * ! Not approriate for small data sets !
  */
  lprintf(stderr,"2- Z-SCORE\n");
  int j=0;
  double zlimit=2.0f;
  lprintf(stderr,"(print z-score greater (in abs) than %lf ...)\n",zlimit);
  for(j;j<i;j++){
    double Zscore=(visamp[j]-mean)/stdev_mean;
    if (fabs(Zscore)>zlimit){
      lprintf(stderr,"%d: amp=%f, Z=%lf\n",j,visamp[j],Zscore);
    };
  };   
  lprintf(stderr,"\n");  
   ...
   * if |M| > 3.5 => outlier 
  */
  lprintf(stderr,"3- MODIFIED Z-SCORE\n");
  int m=0;
  double mzlimit=2.0f;
  lprintf(stderr,"(print z-score greater  (in abs) than %lf ...)\n",mzlimit);
  for(m;m<i;m++){
	double MZscore=(0.6745*(visamp[m]-median))/MAD;
    if (fabs(MZscore)>mzlimit){
      lprintf(stderr,"%d: amp=%f, MZ=%lf\n",m,visamp[m],MZscore);
    };   
  };
  lprintf(stderr,"\n");
 
  /* 4- BOXPLOT METHOD (Tuckey's method) */
  lprintf(stderr,"4- BOXPLOT METHOD\n");  
  Q1 = gsl_stats_quantile_from_sorted_data(visamp,1,i,0.25);
  Q3 = gsl_stats_quantile_from_sorted_data(visamp,1,i,0.75);
  IQR = Q3-Q1;
  lprintf(stderr,"Q1=%lf, Q3=%lf\n",Q1,Q3);
  lprintf(stderr,"Q1-1.5*IQR=%lf, Q3+1.5*IQR=%lf\n",Q1-1.5*IQR,Q3+1.5*IQR);
  lprintf(stderr,"Q1-3*IQR=%lf, Q3+3*IQR=%lf\n",Q1-3*IQR,Q3+3*IQR);
  lprintf(stderr,"\n");  

  /* 5- ADJUSTED BOXPLOT METHOD */
  /* !!! USED !!! -> Best method ? */
  lprintf(stderr,"5- ADJUSTED BOXPLOT METHOD\n");  
  
  /* a. Sort the data (Done!)*/
  
  /* b. Calculate the medcouple (MC)
  * which is a robust measure of the skewness of the distribution
  * (MC>0 indicates right skew and MC<0 left skew) */
  double mcres;
  long s=(long)i;
  
  mcres = mc(visamp,&s);
  lprintf(stderr,"MC = %lf\n",mcres);
  
  /* c. Calculate the lower/upper fence as 
   * [L,U] = [ Q1-1.5*exp(-3.5MC)*IQR , Q3+1.5*exp(4MC)*IRQ ]  if MC >=0
   *       = [ Q1-1.5*exp(-4MC)*IQR , Q3+1.5*exp(3.5MC)*IRQ ]  if MC <=0
  */
  double lowerfence,upperfence;
  
  if (mcres >= 0){
    lowerfence = Q1-1.5*exp(-3.5*mcres)*IQR;
    upperfence = Q3+1.5*exp(4*mcres)*IQR;
  }
  else{
    lowerfence = Q1-1.5*exp(-4*mcres)*IQR;
    upperfence = Q3+1.5*exp(3.5*mcres)*IQR;	  
  }
  lprintf(stderr,"lowerfence=%lf, upperfence=%lf\n",lowerfence,upperfence);
  lprintf(stderr,"\n");  
 
  /* 6- MEDIAN RULE */
  lprintf(stderr,"6- MEDIAN RULE\n");
  
  double mrconstant=2.3; /* This constant is chosen arbitrary */
  /* See Carling, K. Resistant outlier rules and the non-Gaussian case. Computational statistics 
   * and data analysis, vol 33, 2000, pp 249-258.
   */
  
  lprintf(stderr,"median=%lf\n",median);
  lprintf(stderr,"+/- %lf*IQR = %lf ; %lf\n",mrconstant,median-mrconstant*IQR,median+mrconstant*IQR);
  lprintf(stderr,"\n");  
 
  /* asymetrie : moment d'ordre 3 */
  /* The skewness measures the asymmetry of the tails of a distribution. */
  /* <0 : asymetrique ; queue de distribution etalee a gauche  */ 
  /* =0 : symetrique */ 
  /* >0 : asymetrique ; queue de distribution etalee a droite */ 
  /* skew = gsl_stats_skew(visamp,1,i);*/
  /* skew = gsl_stats_skew_m_sd(visamp,1,i,mean,stdev_mean); */
  
  /* aplatissement : moment d'ordre 4 */
  ...
  /* Use ? */
  /* kurtosis = gsl_stats_kurtosis(visamp,1,i); */
  /*   kurtosis = gsl_stats_kurtosis_m_sd(visamp,1,i,mean,stdev_mean); */
  
  /*lprintf(stderr,"skew=%lf, kurtosis=%lf\n",skew,kurtosis);*/
  
  /* Absolute deviation from any given value */
  /* gsl_stats_absdev_m(data,stride,n,value) */
  
  
  
  /*lprintf(stderr,"\n* Phase error\n");
  meanphserr = gsl_stats_mean(visphserr,1,i);
  stdev_meanphserr = gsl_stats_sd_with_fixed_mean(visphserr,1,i,meanphserr);
  gsl_sort(visphserr,1,i);
  medianphserr = gsl_stats_median_from_sorted_data (visphserr, 1, i);
  stdev_medianphserr = gsl_stats_sd_with_fixed_mean(visphserr,1,i,medianphserr);
  lprintf(stderr,"mean=%lf, sigma (mean)=%lf, median=%lf, sigma (med)=%lf\n",meanphserr,stdev_meanphserr,medianphserr,stdev_medianphserr);
  */
...
/* Effective flagging */
  for(t=vp->ta; !ierr && t<=vp->tb; t++) {
    ...
    if(tval >= tmin && tval <= tmax) {  /* Only flag in given time range */
      ...
      tmp_amperr = 1.0f/sqrt(fabs(vis->wt));
      
      tmp_phs -= twopi*floor(vis->phs/twopi+0.5);  /* Wrap into -pi to pi range */
      if(vis->amp > 1.0e-20f){
	tmp_phserr = tmp_amperr/vis->amp;
	/*lprintf(stderr,"Phserr =%f\n",tmp_amperr/vis->amp);*/
      }	
      else
        tmp_phserr = 0.0f;
 ...
 */
	...
	continue;
/*
 * Edit and redisplay the point. UNDER CERTAIN CONTIDIONS ??
 * For the moment, use the adjusted boxplot method! (see above)
 */
      if (vis->amp > upperfence || vis->amp < lowerfence) {
        ierr = v_edit(vp, vs, doflag, t);
      };
	...
	continue;
      ...
      /*if (tmp_phserr > meanphserr+3.0f*stdev_meanphserr || tmp_phserr < meanphserr-3.0f*stdev_meanphserr) {
        ierr = v_edit(vp, vs, doflag, t);
      };
      */
  ...
  vp->stat_ed = save_mode; /* Restore station/baseline editing mode. */
  vp->if_ed = save_if_mode; /* Restore all/single IF editing mode. */
  cpgebuf(); /* Plotting complete - end plot buffering */
  return ierr;
}
 ...
 *  return      int    0 - OK.
 */
int v_edit(Vedpar *vp, Vissub *vs, int flag, int t)
{
 ...
 */
	...
	case KEY_ZAP:     /* Zap all points of a given scan/baseline */
	  ...
	  break;
	/* A.Collioud - 2014/03/20 */
	case KEY_SMARTFLG:     /* Smart flag of a given scan/baseline */
	  ierr = v_smart(vp, vp->cursor.vs, vp->cursor.scan, 1);
	  break;
	/* ------------------------*/  
	case KEY_CROSS:
	  vp->docross = !vp->docross;
	...
	case KEY_HELP:    /* Print usage info */
	  ...
	  /* A.Collioud - 2014/03/20 */	  
	  printf(" %c - Smart automatic flag of a selected baseline and scan.\n", KEY_SMARTFLG);
	  /* ------------------------*/  
 ...
 *  dobars      int    If true plot error bars.
 *  showall     int    If true, take account of flagged data in autoscaling.
 *  coefsigma  float   Number of sigma as cutoff for visibility amplitudes.
 *  nbvislimit  int    Number of visibilities in a scan to proceed to smart editing.
 ...
 *  return      int    0 - OK.
 *
 * A.Collioud - collioud@obs.u-bordeaux1.fr - 2014 March 26
 * !! Copy of vedit !!
 */
int smarted(Observation *ob, Basespec *bs, int cif, int nrow, int npage,
	  int docurs, char *opts, int doscan, int doamp, int dophs,
	  int doflag, int domod, int dobars, int showall, int *modified, int *method, float *coefsigma, int *nbvislimit, int *verbose)
{
  Vedpar *vp;     /* Plot descriptor */
  int ierr=0;     /* Error status */
  int finished=0; /* True when user quits */
  ...
  int flag_verbose=*verbose;    /* Verbose output switch. */
 ...
 * Is the observation ready to be plotted?
 */
  if(!ob_ready(ob, OB_SELECT, "smartedit"))
    return 1;
/*
 * If no baseline specification has been provided, override with the
 * default.
 ...
 * Plot the first page.
 */
  ierr = v_plot_smart(vp, V_ALLNEW, 1, bs, &method, &coefsigma, &nbvislimit) <= 0;
 ...
 */
    for(page=1; !ierr && (npage<=0 || page<npage); page++) {
      int nplotted = v_plot_smart(vp, V_NEXT, 1, NULL, &method, &coefsigma, &nbvislimit);
      if(nplotted < 0)
	ierr = 1;
      else if(nplotted == 0)
	break;
