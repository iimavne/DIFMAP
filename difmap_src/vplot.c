...
/*
 * In non-interactive mode tell the user what is being plotted (if verbose option is on).
 */
  if(!vp->docurs) {
	/* Only 1 plot on display. Typically for smartedit */
	if ((vp->bs_beg.ta == vp->bs_end.ta) && (vp->bs_beg.tb == vp->bs_end.tb)){
		
		/* No print if no data? */
    /* Too much print --> commented */
			/*lprintf(stdout, "Page %02.2d: %s %d:%s-%s (IF %d)\n", vp->npage,
			vp->doall ? "Baseline":"Upper baseline",
			vp->bs_beg.isub+1, sub->tel[vp->bs_beg.ta].name,sub->tel[vp->bs_end.tb].name,vp->ob->stream.cif+1);
      */
 ...
 *  ( verbose     int    Verbose output. => in vedpar object )
 * 
 * Output:
 *  return      int    The number of sub-plots plotted. Or -1 on
 *                     error.
 ...
 * !! Copy of v_plot !!
 */
int v_plot_smart(Vedpar *vp, Vedop oper, int forward, Basespec *init, int **meth, float **coefsig, int **nblimit)
{
  Observation *ob; /* The descriptor of the parent observation */
  Basespec bs;     /* The new trial baseline specification */
  
  int method=**(meth);			 /* Method to detect outliers (default by standard deviation) */
  float coefsigma=**(coefsig);   /* Number of sigma as cutoff for visibility amplitudes. */
  int nbvislimit=**(nblimit);    /* Number of visibilities in a scan to proceed to smart editing. */
  
/*
 * Check arguments.
 */
  if(vp==NULL) {
    lprintf(stderr, "v_plot_smart: NULL plot descriptor intercepted\n");
    return -1;
  };
/*
 * Get the descriptor of the parent observation.
 ...
 */
  if(vp->nplot<1 && oper!=V_ALLNEW) {
    lprintf(stderr, "v_plot_smart: First call must use V_ALLNEW.\n");
    return -1;
  };
/*
 * Perform any positioning operations required for the particular
 ...
 */
  ...
  case V_ALLNEW:
    if(!init) {
      lprintf(stderr, "v_plot_smart: Invalid baseline specification.\n");
      return -1;
    };
    bs = *init;
    if(next_base(ob, FIND_FIRST, forward, 2, vp->doall, 0, 1, &bs))
  ...
  case V_NEXT:
    ...
    break;
  default:
    lprintf(stderr, "v_plot_smart: Unknown opcode intercepted.\n");
    return -1;
    break;
  };
/*
 ...
 */
  ...
  /* A.Collioud - 9 April 2014*/
  
  int nbscans=nscans(vp->sub,vp->sub->scangap);
  /*lprintf(stdout,"NB SCANS: %d\n",nbscans);*/
  ...
  for (m=0;m<nbscans;m++){
    ...
    for (k=0;k<ob->nif;k++){
      ...
      if(!getIF(ob,k)){  
        /*lprintf(stdout,"IF loaded= %d\n",get_cif_state(ob));*/
      
        Vissub *vs = &vp->vplots[0];    /* Only one sub-plot at a time */

        int base=vp->bs_beg.base; /* Baseline number (same as bs_end, since only one baseline plotted) */           
        int t;                    /* Time sample */
        ...
        double cutoff_down=0;		/* Visibility amplitude cutoffs */
        double cutoff_up=10;		/* Set default to avoid 'all editing' if error in cutoffs calculation */
                   
        int i=0;   /* Counter of visibilities */
        double visamp[vp->tb]; /* Visibility amplitude vector. NB: vp->tb + 1 = nb of visibilities in the scan */
        double visamperr[vp->tb]; /* Visibility amplitude error vector */

       /*
        * Get the time values of the start / end of the scan.
        */
        tmin = vp->scans[m].tmin;
        tmax = vp->scans[m].tmax;
        ...
        for(t=vp->ta; t<=vp->tb; t++) {
          ...
          */
           ...
           */
            ...
            if(vis->bad){
              continue;
            }; /* this allows mutiple run of smart editing */
        ...
        /* Calculate statistics, then editing
         * 
         * NB: only if the number of visibilities is > nbvislimit (which is >0))
		 */
        if (i>nbvislimit){

			/* General stats */
			gsl_sort(visamp,1,i); /* sort data */			
			gsl_stats_minmax (&min, &max, visamp, 1, i);
			mean = gsl_stats_mean(visamp,1,i);
			stdev = gsl_stats_sd_with_fixed_mean(visamp,1,i,mean);
			median = gsl_stats_median_from_sorted_data (visamp, 1, i);
			
			if (vp->verbose){
				lprintf(stdout,"Nb of  vis = %d\n",i);
				lprintf(stdout,"Min = %lf ; Max = %lf\n",min,max);
				lprintf(stdout,"Mean = %lf ; stdev = %lf\n",mean,stdev);
				lprintf(stdout,"Median = %lf\n",median);
				
				int j=0;
				lprintf(stdout,"Data:\n");
				for(j;j<i;j++){
					/*lprintf(stdout,"vis(%d): amp=%lf (err %lf)\n",j,visamp[j],visamperr[j]);*/  
					lprintf(stdout,"%.*f, ",16, visamp[j]); 
				};
		    };
		...
		/* #1 ST_DEVIATION METHOD */
			if (method==1){

				/* Cutoffs */
				cutoff_down = mean - coefsigma*stdev;
				cutoff_up = mean + coefsigma*stdev;
				
				if (vp->verbose){
					lprintf(stdout,"Mean +/- n * sigma\n");				 
					lprintf(stdout,"%lf +/- %lf * %lf\n",mean,coefsigma,stdev);				 					
					lprintf(stdout,"*** Cutoffs = [ %lf, %lf ] ***\n",cutoff_down,cutoff_up);
				};
			};
		...
		/* #2 MADe METHOD */
			...
			if (method==2){
				...
				MADe = 1.4826*MAD;  
				/* 1.4826 is required for consistency with normal distribution */
				
				/* Cutoffs */
				cutoff_down = median - coefsigma*MADe;
				cutoff_up = median + coefsigma*MADe;
				
				if (vp->verbose){
					lprintf(stdout,"Median +/- n * MADe\n");				 
					lprintf(stdout,"%lf +/- %lf * %lf\n",median,coefsigma,MADe);				 
					lprintf(stdout,"*** Cutoffs = [ %lf, %lf ] ***\n",cutoff_down,cutoff_up);
				};
			};
		...
		/* #3 Z-SCORE */
		   /* Z = (xi-mean)/stdev
		   * normally, if |Z| > 3 => outlier (equivalent to 3 sig)
		   * ! Not approriate for small data sets !
		   */
			if (method==3){
		    
				/* Calculation done on the editing part of the function (see below) */
				
				/*int j=0;
				for(j;j<i;j++){
				  double Zscore=(visamp[j]-mean)/stdev;
				  if (fabs(Zscore)>=coefsigma){
					lprintf(stderr,"amp=%f, Z=%lf\n",visamp[j],Zscore);
				  };
				};*/   
		    };  
		    ...
		    */
			if (method==4){
				...
				for(m;m<i;m++){
				  ...
				  if (fabs(MZscore)>=coefsigma){
					lprintf(stderr,"amp=%f, MZ=%lf\n",visamp[m],MZscore);
				  };   
				};*/
		    };
		 
		/* #5 BOXPLOT METHOD (Tuckey's method) */
			if (method==5){
				...
				/* Cutoffs */
				cutoff_down = Q1-coefsigma*IQR;
				cutoff_up = Q3+coefsigma*IQR;
							
				if (vp->verbose){
					lprintf(stdout,"Q1 = %lf, Q3 = %lf --> IRQ = %lf\n",Q1,Q3,IQR);
					lprintf(stdout,"Q1 - %lf IQR ; Q3 + %lf IQR ;\n",coefsigma,coefsigma);
					lprintf(stdout,"*** Cutoffs = [ %lf, %lf ] ***\n",cutoff_down,cutoff_up);
				};
			};
  		/* #6 ADJUSTED BOXPLOT METHOD */
			if (method==6){
				...
				* (MC>0 indicates right skew and MC<0 left skew) */
				...
				mcres = mc(visamp,&s);
				if (vp->verbose){
					lprintf(stdout,"Medcouple (MC) = %lf\n",mcres);
				};
				
				/* c. Calculate quartiles Q1, Q3 and IQR */
				double Q1,Q3,IQR;
				Q1 = gsl_stats_quantile_from_sorted_data(visamp,1,i,0.25);
				Q3 = gsl_stats_quantile_from_sorted_data(visamp,1,i,0.75);
				IQR = Q3-Q1;
				
				/* d. Calculate the lower/upper fence as: 
				 * [L,U] = [ Q1-n*exp(-3.5MC)*IQR , Q3+n*exp(4MC)*IRQ ]  if MC >=0
				 *       = [ Q1-n*exp(-4MC)*IQR , Q3+n*exp(3.5MC)*IRQ ]  if MC <=0
				*/
				
				if (mcres >= 0){
				  cutoff_down = Q1-coefsigma*exp(-3.5*mcres)*IQR;
				  cutoff_up = Q3+coefsigma*exp(4*mcres)*IQR;
				}
				else{
				  cutoff_down = Q1-coefsigma*exp(-4*mcres)*IQR;
				  cutoff_up = Q3+coefsigma*exp(3.5*mcres)*IQR;	  
				}
				
				if (vp->verbose){
					lprintf(stdout,"Q1 = %lf, Q3 = %lf --> IRQ = %lf\n",Q1,Q3,IQR);
					if (mcres >= 0){
						lprintf(stdout,"Q1 - %lf*exp(-3.5MC)*IQR ; Q3 + %lf*exp(4MC)*IQR\n",coefsigma,coefsigma);
					}
					else{
						lprintf(stdout,"Q1 - %lf*exp(-4MC)*IQR ; Q3 + %lf*exp(3.5MC)*IQR\n",coefsigma,coefsigma);					
					}
					lprintf(stdout,"*** Cutoffs = [ %lf, %lf ] ***\n",cutoff_down,cutoff_up);
				};
			};
		...
		/* #7- MEDIAN RULE */		  
			if (method==7){
				...
				/* Cutoffs */
				cutoff_down = median-coefsigma*IQR;
				cutoff_up = median+coefsigma*IQR;
				
				/* NB:
				 * In this case, a constant value of 2.3 is a good compromise.
				...
				if (vp->verbose){
					lprintf(stdout,"Median +/- n * IQR\n");				 
					lprintf(stdout,"%lf +/- %lf * %lf\n",median,coefsigma,IQR);				 
					lprintf(stdout,"*** Cutoffs = [ %lf, %lf ] ***\n",cutoff_down,cutoff_up);
				};
		    };		
				...
				if(tval >= tmin && tval <= tmax) {
					...
					*/
					if(vis->bad & FLAG_DEL)
						continue;
					/*
					* Edit "outliers" and redisplay the point.
					*/
					if (method==3){	/* z-score */
						double Zscore=fabs((vis->amp-mean)/stdev);
						
						if (Zscore >= coefsigma){
							v_edit(vp, vs, 1, t);
						};
					}
					else if (method==4){  /* modified z-score */
						double MZscore=fabs((0.6745*(vis->amp-median))/MAD);
						
						if (MZscore >= coefsigma){
							v_edit(vp, vs, 1, t);
						};
					}
					else {	/* all other methods */
						if (vis->amp > cutoff_up || vis->amp < cutoff_down) {
							v_edit(vp, vs, 1, t);
						};
					}
		...
		/*
		* Display the new baselines. [MANDATORY TO VALIDATE EDITING]
		*/
		if(v_redisp(vp))
			return -1;
